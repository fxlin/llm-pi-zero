...
# xzl: 
# The Touchkeytrackid array in the GT_Development class is used to store the
# unique identifier for each touch point detected on the touch screen
# ... for tracking continuous move....
...
try:
    ...
    DrawImage = ImageDraw.Draw(image)   # xzl: for future drawing?
    ...
    # time.sleep(2)
    # epd.init(epd.PART_UPDATE)
    draw.text((8, 36), u'????', font = font15, fill = 0)    
    epd.init(epd.PART_UPDATE)
    # --- ok ---- #
    ...
    while(1):
        ...
        # xzl: below, both GT_Dev and GT_Old are updated (GT_Old: prev touch points
        # xzl: why need GT_Old?? check if the touch point has changed?
        # x,y,s are the touch point and size (?), thre are 5 touch points (max)    
